...
class MSHybridAEDirectLoss(nn.Module):
    ...
    Uses L1 reconstruction loss for speech (and optionally music),
    and optional Mel Spectrogram loss for speech.
    SI-SNR for speech and music are calculated for reporting purposes but not for direct backprop.
    Designed specifically for the MSHybridNet autoencoder architecture.
    """
    def __init__(self, mel_loss_weight=0.1, # Removed mix_recon_weight
        ...
        super().__init__()
        # self.mix_recon_weight = mix_recon_weight # Removed
        ...
        # self.mix_recon_loss_fn = nn.L1Loss() # Removed
        self.l1_loss_fn = nn.L1Loss() # L1 loss for individual sources

    def to(self, device):
        super().to(device)
        if self.mel_loss_fn is not None:
            self.mel_loss_fn = self.mel_loss_fn.to(device)
        self.l1_loss_fn = self.l1_loss_fn.to(device) # Ensure L1 loss is moved
        return self

    def forward(self, s_estimates, targets, mix_estimate, mix_target): # mix_estimate and mix_target are no longer used for loss
        # s_estimates: (Batch, NumSources, Channels, Time)
        # targets: (Batch, NumSources, Channels, Time)
        # mix_estimate: (Batch, Channels, Time) - Not used for loss here
        # mix_target: (Batch, Channels, Time) - Not used for loss here
        batch_size, num_sources, num_channels, _ = s_estimates.shape
        device = s_estimates.device
        
        # Ensure all inputs are on the same device
        targets = targets.to(device)
        # mix_estimate = mix_estimate.to(device) # Not used for loss
        # mix_target = mix_target.to(device)     # Not used for loss
        ...
        if num_channels == 1:
            ...
            # mix_estimate_mono = mix_estimate.squeeze(1) # (B, T) # Not used
            # mix_target_mono = mix_target.squeeze(1)     # (B, T) # Not used
            ...
            # mix_estimate_mono = torch.mean(mix_estimate, dim=1) # Not used
            # mix_target_mono = torch.mean(mix_target, dim=1)     # Not used
        ...
        # 1. Speech L1 Loss (for gradient)
        loss_l1_speech = self.l1_loss_fn(speech_est, speech_tgt) # Scalar mean

        # 2. Music L1 Loss (potentially for gradient)
        loss_l1_music = torch.tensor(0.0).to(device)
        if self.include_music_loss:
            loss_l1_music = self.l1_loss_fn(music_est, music_tgt) # Scalar mean

        # 3. Mixture Reconstruction Loss (REMOVED)
        ...
        total_loss = loss_l1_speech # Start with speech L1 loss
        if self.include_music_loss:
            total_loss += loss_l1_music # Add music L1 loss if requested
        # total_loss += self.mix_recon_weight * loss_mix_recon # Removed
        total_loss += self.mel_loss_weight * loss_mel_speech
        
        # --- Calculate SI-SNR for reporting (not for backprop) ---
        avg_neg_sisnr_speech = torch.mean(si_snr_loss_manual(speech_est, speech_tgt))
        avg_neg_sisnr_music = torch.mean(si_snr_loss_manual(music_est, music_tgt))

        # Return total loss for backprop and components for logging/evaluation
        # Removed loss_mix_recon, added loss_l1_speech, loss_l1_music, and SI-SNR values for reporting
        return total_loss, loss_l1_speech, loss_mel_speech, loss_l1_music, avg_neg_sisnr_speech, avg_neg_sisnr_music
...
def evaluate(model, dataloader, loss_fn_eval, metrics_dict, device, desc="Evaluating", max_batches=0):
    ...
    model.eval()
    # Renamed and removed loss accumulators
    total_loss_l1_speech = 0.0
    total_loss_l1_music = 0.0
    # total_loss_mix_recon = 0.0 # Removed
    total_loss_mel = 0.0
    num_samples = 0
    ...
    with torch.no_grad():
        ...
        for batch_idx, (mixture, target_speech, target_music) in enumerate(eval_pbar):
            ...
            # Unpack new return values from loss_fn_eval
            _, batch_loss_l1_speech, batch_loss_mel_speech, batch_loss_l1_music, \
            batch_avg_neg_sisnr_speech, batch_avg_neg_sisnr_music = loss_fn_eval(
                s_estimates, targets, x_mix_recon, mixture_for_loss # Pass original mixture for loss, x_mix_recon not used by loss now
            )
            # Accumulate new L1 losses
            total_loss_l1_speech += batch_loss_l1_speech.item() * batch_size
            if loss_fn_eval.include_music_loss: # only accumulate if music loss is active in loss_fn_eval
                total_loss_l1_music += batch_loss_l1_music.item() * batch_size
            
            # total_loss_mix_recon += batch_loss_mix_recon.item() * batch_size # Removed
            total_loss_mel += batch_loss_mel_speech.item() * batch_size
            
            # SI-SNR metrics are now based on the explicitly calculated neg_sisnr values
            accumulated_music_sisnr_metric += (-batch_avg_neg_sisnr_music.item()) * batch_size 
            accumulated_speech_sisnr_metric += (-batch_avg_neg_sisnr_speech.item()) * batch_size
    ...
    if num_samples > 0:
        ...
        avg_loss_l1_music = total_loss_l1_music / num_samples if loss_fn_eval.include_music_loss and num_samples > 0 else 0.0
        # avg_loss_mix_recon = total_loss_mix_recon / num_samples # Removed
        avg_loss_mel = total_loss_mel / num_samples
        
        # Update results dictionary with new loss names
        results["Loss_Speech(L1)"] = avg_loss_l1_speech
        if loss_fn_eval.include_music_loss:
            results["Loss_Music(L1)"] = avg_loss_l1_music
        # results["Loss_Mix(L1)"] = avg_loss_mix_recon # Removed
        results["Loss_Mel"] = avg_loss_mel
        
        # Calculate total loss based on eval set averages (more stable)
        # results["Loss_Total"] = avg_loss_sisnr_speech + loss_fn_eval.mix_recon_weight * avg_loss_mix_recon + loss_fn_eval.mel_loss_weight * avg_loss_mel # Old
        results["Loss_Total"] = avg_loss_l1_speech + loss_fn_eval.mel_loss_weight * avg_loss_mel
        if loss_fn_eval.include_music_loss:
             results["Loss_Total"] += avg_loss_l1_music
             # results["Loss_Music(SI-SNR)"] = avg_loss_sisnr_music # This was from old SI-SNR loss, now L1
        
        # These are the SI-SNR values calculated for reporting, not the L1 losses used for training
        # The keys Loss_Speech(SI-SNR) and Loss_Music(SI-SNR) might be confusing if not handled carefully
        # Let's ensure that the reported SI-SNR losses are still available if needed elsewhere,
        # but the primary "Loss_..." keys reflect the L1 losses.
        # The evaluate function already calculates SI-SNR for speech and music via accumulated_..._sisnr_metric
        # and these are stored in Speech_SI-SNR_Eval and Music_SI-SNR_Eval.
        # So, the 'Loss_Speech(SI-SNR)' and 'Loss_Music(SI-SNR)' from the previous version are effectively replaced by L1.

        # Add separate speech and music SI-SNR metric values (these are fine, based on accumulated metrics)
        ...
        results["SI-SNR"] = 0.0
        results["Loss_Speech(L1)"] = 0.0 # Changed from SI-SNR
        if getattr(loss_fn_eval, 'include_music_loss', False) : # Check if attribute exists
            results["Loss_Music(L1)"] = 0.0 # Changed from SI-SNR
        # results["Loss_Mix(L1)"] = 0.0 # Removed
...
def plot_losses(train_losses, val_losses, save_path, train_losses_components=None, val_losses_components=None, include_music_loss=False):
    ...
    Args:
        ...
        train_losses_components: Dict of component losses for training (speech_l1, music_l1, mel)
    ...
    if train_losses_components and val_losses_components:
        # Number of components: Speech L1, Mel (always), Music L1 (if included)
        component_plots = 2 # Speech L1, Mel
        if include_music_loss and 'music_l1' in train_losses_components:
            component_plots +=1
    ...
    if component_plots > 0 and train_losses_components and val_losses_components:
        ...
        plot_idx_offset = 2 # Start plotting components from subplot index 2
        ...
        # Speech L1 Loss
        plt.subplot(1 + component_plots, 1, plot_idx_offset)
        ...
        # Music L1 Loss (if included)
        if include_music_loss and 'music_l1' in train_losses_components and 'music_l1' in val_losses_components:
        ...
        # Mel Spectrogram Loss (always last among components)
        plt.subplot(1 + component_plots, 1, plot_idx_offset)
...
def train(args):
    ...
    if hasattr(loss_fn, 'mel_loss_fn'):
        loss_fn.mel_loss_fn = loss_fn.mel_loss_fn.to(device)
    if hasattr(loss_fn, 'l1_loss_fn') and loss_fn.l1_loss_fn is not None:
        loss_fn.l1_loss_fn = loss_fn.l1_loss_fn.to(device)
    ...
    train_l1_s_loss, val_l1_s_loss = [], [] # Added
    train_l1_m_loss, val_l1_m_loss = [], [] # Added
    ...
    train_music_si_snr, val_music_si_snr = [], [] # For positive SI-SNR metrics (reporting)
    ...
    if latest_checkpoint_info:
        ...
        try:
            ...
            train_l1_s_loss = checkpoint.get('train_l1_s_loss', [])
            val_l1_s_loss = checkpoint.get('val_l1_s_loss', [])
            train_l1_m_loss = checkpoint.get('train_l1_m_loss', [])
            val_l1_m_loss = checkpoint.get('val_l1_m_loss', [])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        epoch_loss_l1_speech = 0.0    # Added
        epoch_loss_l1_music = 0.0     # Added
        epoch_loss_mel = 0.0
        # Accumulators for SI-SNR values (for reporting, from loss_fn)
        ...
        for i, (mixture, target_speech, target_music) in enumerate(train_pbar):
            ...
            batch_total_loss, batch_l1_speech, batch_mel_speech, batch_l1_music, \
            batch_avg_neg_sisnr_speech, batch_avg_neg_sisnr_music = loss_fn(
                s_estimates, targets, x_mix_recon, mixture_target # x_mix_recon not used by loss
            )
            ...
            epoch_loss_l1_speech += batch_l1_speech.item() * batch_size # Added
            if args.include_music_loss:
                epoch_loss_l1_music += batch_l1_music.item() * batch_size # Added

            epoch_neg_sisnr_speech_report += batch_avg_neg_sisnr_speech.item() * batch_size # For reporting
            epoch_neg_sisnr_music_report += batch_avg_neg_sisnr_music.item() * batch_size  # For reporting
            
            epoch_loss_mel += batch_mel_speech.item() * batch_size
        ...
        avg_epoch_loss_l1_speech = epoch_loss_l1_speech / num_train_samples     # Added
        avg_epoch_loss_l1_music = epoch_loss_l1_music / num_train_samples if args.include_music_loss and num_train_samples > 0 else 0.0 # Added
        ...
        train_l1_s_loss.append(avg_epoch_loss_l1_speech) # Added
        if args.include_music_loss:
            train_l1_m_loss.append(avg_epoch_loss_l1_music) # Added
        else:
            train_l1_m_loss.append(0.0) # Append placeholder if not used, for consistent list length with epochs

        train_mel_loss.append(avg_epoch_loss_mel)
        train_speech_si_snr.append(-avg_epoch_neg_sisnr_s_report) # Convert reported neg SI-SNR to positive SI-SNR metric
        ...
        val_l1_s_loss.append(val_results['Loss_Speech(L1)'])     # Added
        if args.include_music_loss and 'Loss_Music(L1)' in val_results:
            val_l1_m_loss.append(val_results['Loss_Music(L1)'])   # Added
        elif args.include_music_loss: # If key missing but should be there
             val_l1_m_loss.append(0.0) # Append placeholder
        else: # if not include_music_loss
             val_l1_m_loss.append(0.0) # Append placeholder for consistent list length
        ...
        print(f"       Speech L1: {avg_epoch_loss_l1_speech:.4f} | Music L1: {avg_epoch_loss_l1_music:.4f} | Mel: {avg_epoch_loss_mel:.4f}") # Updated
        print(f"       Speech SI-SNR (Report): {-avg_epoch_neg_sisnr_s_report:.2f} dB | Music SI-SNR (Report): {-avg_epoch_neg_sisnr_m_report:.2f} dB") # Added "Report"
        ...
        val_l1_s_print = val_results.get('Loss_Speech(L1)', 0.0)
        val_l1_m_print = val_results.get('Loss_Music(L1)', 0.0) if args.include_music_loss else "N/A"
        val_mel_print = val_results.get('Loss_Mel', 0.0)
        if isinstance(val_l1_m_print, float): val_l1_m_print = f"{val_l1_m_print:.4f}" # format if number
        print(f"       Speech L1: {val_l1_s_print:.4f} | Music L1: {val_l1_m_print} | Mel: {val_mel_print:.4f}") # Updated
        ...
        writer.add_scalar('Loss/train_speech_l1', avg_epoch_loss_l1_speech, epoch+1) # New
        if args.include_music_loss:
            writer.add_scalar('Loss/train_music_l1', avg_epoch_loss_l1_music, epoch+1) # New
        writer.add_scalar('Loss/train_mel', avg_epoch_loss_mel, epoch+1)

        # Log performance metrics (positive values for SI-SNR)
        writer.add_scalar('Metrics/train_speech_sisnr', -avg_epoch_neg_sisnr_s_report, epoch+1) # Use reported SI-SNR
        writer.add_scalar('Metrics/train_music_sisnr', -avg_epoch_neg_sisnr_m_report, epoch+1)  # Use reported SI-SNR
        ...
        writer.add_scalar('Loss/val_speech_l1', val_results['Loss_Speech(L1)'], epoch+1) # New
        # Log music SI-SNR loss if included in training # Old comment
        if args.include_music_loss and 'Loss_Music(L1)' in val_results: # Changed to L1
            writer.add_scalar('Loss/val_music_l1', val_results['Loss_Music(L1)'], epoch+1) # New
        ...
        fig = plot_losses(train_losses_total, val_losses_total, output_dir / f'loss_plot_epoch_{epoch+1}.png', {
            'speech_l1': train_l1_s_loss, # New
            'music_l1': train_l1_m_loss if args.include_music_loss and train_l1_m_loss else [], # New
            'mel': train_mel_loss
        }, {
            'speech_l1': val_l1_s_loss, # New
            'music_l1': val_l1_m_loss if args.include_music_loss and val_l1_m_loss else [], # New
            'mel': val_mel_loss
    ...
    if best_model_path.exists():
        ...
        try:
            ...
            print(f"  Speech L1:        {test_results.get('Loss_Speech(L1)', 0.0):.4f}") # New
            if args.include_music_loss: 
                print(f"  Music L1:         {test_results.get('Loss_Music(L1)', 0.0):.4f}") # New
            ...
            writer.add_scalar('Test/Loss_Speech_L1', test_results.get('Loss_Speech(L1)',0.0), 0) # New
            if args.include_music_loss and 'Loss_Music(L1)' in test_results:
                writer.add_scalar('Test/Loss_Music_L1', test_results['Loss_Music(L1)'], 0) # New
            ...
            test_summary = (
                ...
                f"Speech L1: {test_results.get('Loss_Speech(L1)', 0.0):.4f}\n"
                f"Music L1: {test_results.get('Loss_Music(L1)', 0.0):.4f}\n"
            )
            if pesq_key in test_results:
